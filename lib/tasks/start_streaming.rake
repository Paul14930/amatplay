...
#     stream_command = <<-CMD
#       ffmpeg -rtsp_transport tcp -i "#{camera_rtsp_url}" -s 640x480 -vcodec libx264 -preset veryfast -crf 23 -acodec aac -strict -2 -f hls -hls_time 2 -hls_list_size 5 -hls_wrap 10 #{Rails.root.join('public', 'stream.m3u8')}
#     CMD
#     puts "Running command: #{stream_command}"
#     system(stream_command)
#   end
